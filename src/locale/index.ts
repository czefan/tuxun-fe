...
export function getTemplateByKey(key: string) {
  ...
  const message = (messages as Record<string, any>)[locale] || messages['zh-Hans']
  ...
  try {
    ...
    const result = keyList.reduce((pre: any, cur) => {
...
function formatI18n(template: string, data?: any) {
  ...
  return template.replace(/\{([^}]+)\}/g, (match, key: string) => {
    ...
    while (arr.length) {
      ...
      if (first !== undefined && result !== undefined && result !== null) {
        result = result[first]
      }
      else {
        result = ''
